import { CategoryModel } from "../../data";
import { CategoryEntity, CreateCategoryDto, CustomError, PaginationDto, UserEntity } from "../../domain";



export class CategoryService {


    constructor(){}


    async createCategory( createCategoryDto: CreateCategoryDto, user: UserEntity ){

        const categoryExists = await CategoryModel.findOne({ name: createCategoryDto.name });
        if( categoryExists ) throw CustomError.basRequest('Category already exists');


        try {
            
            const category = new CategoryModel({
                ...createCategoryDto,
                user: user.id
            })

            await category.save();

            const { name, available, id } = category;

            return {
                id,
                name,
                available
            }


        } catch (error) {
            throw CustomError.internalServer(`Internal server error ${error}`)
        }

    };

    async getCategories( paginationDto: PaginationDto ){

        const { page, limit } = paginationDto;

        try {

            const [total, categories] = await Promise.all([
                 CategoryModel.countDocuments(),
                 CategoryModel.find()
                .skip( (page - 1) * limit )
                .limit( limit )
            ])

            const categoryEntities = categories.map( category => CategoryEntity.fromObject( category ) );

            console.log(categoryEntities);
            
            
            return {
                page,
                limit,
                total,
                next: `/api/categories?page=${ (page+1) }&limit=${ limit }`,
                previous: (page - 1 >  0) ? `/api/categories?page=${ page - 1 }&limit=${ limit }`: null,

                categories: categoryEntities
            }
            
        } catch (error) {
            throw CustomError.internalServer(`Error al traer las categorias activas. ${error}`)
        }


        // [{},{},{},]
    }


}


